...
$sql_comments="SELECT * FROM comments WHERE status='1' ORDER BY id DESC";
$results_comment= mysqli_query($conn,$sql_comments);
...
<div id="news">
	...
	<?php 
	while($data_comments= mysqli_fetch_assoc($results_comment)){
	?>
	<div class="news wide "> 
		<div class="comment_name alignleft">
			<h3 style=""><?php echo $data_comments['name']; ?></h3>
			<h6>Mobile: <?php echo $data_comments['phone']; ?></h6>
			<h6>Email: <?php echo $data_comments['email']; ?></h6>
		</div>
	        
        <div  class="right" >
			<p style="color:#000; margin-top:30px;">
            	<?php echo $data_comments['comment']; ?>
            </p>
        </div>
    </div>
    <?php }?>
